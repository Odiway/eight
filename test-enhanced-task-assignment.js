...
function testSearchFunctionality() {
  ...
  console.log(
    `✅ Search "ahmet": Found ${nameResults.length} users - ${nameResults
      .map((u) => u.name)
      .join(', ')}`
  )
  ...
  console.log(
    `✅ Search "yazılım": Found ${deptResults.length} users - ${deptResults
      .map((u) => u.name)
      .join(', ')}`
  )
  ...
  console.log(
    `✅ Search "developer": Found ${posResults.length} users - ${posResults
      .map((u) => u.name)
      .join(', ')}`
  )
  ...
  console.log(
    `✅ Multi-search "test": Found ${multiResults.length} users - ${multiResults
      .map((u) => u.name)
      .join(', ')}`
  )
}
...
function testUserSelection() {
  ...
  const addUser = (userId) => {
    if (!selectedUserIds.includes(userId)) {
      ...
      console.log(
        `✅ Added user: ${mockUsers.find((u) => u.id === userId)?.name}`
      )
    }
  }
  ...
  const removeUser = (userId) => {
    ...
    console.log(
      `✅ Removed user: ${mockUsers.find((u) => u.id === userId)?.name}`
    )
  }
...
function testValidation() {
  ...
  console.log(
    `✅ Valid task data: ${validErrors.length} errors - ${
      validErrors.join(', ') || 'No errors'
    }`
  )
