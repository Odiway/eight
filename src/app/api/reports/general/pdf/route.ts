  ...
  try {
    ...
    const generalData = {
      ...
      stats: {
        ...
        completedTasks: projects.reduce((sum, p) => sum + (p.tasks?.filter((t: any) => t.status === 'COMPLETED').length || 0), 0),
        totalUsers: users.length
      },
      departments: Object.entries(
        users.reduce((acc: any, user) => {
          if (!acc[user.department]) {
            acc[user.department] = { name: user.department, userCount: 0, projectCount: 0 };
          }
...
function generatePDF(data: any) {
  ...
  pdf.setFontSize(10);
  pdf.text(formatTurkishText(`Toplam Proje: ${data.stats.totalProjects}`), 25, yPosition);
  yPosition += 8;
  pdf.text(formatTurkishText(`Toplam Gorev: ${data.stats.totalTasks}`), 25, yPosition);
  yPosition += 8;
  pdf.text(formatTurkishText(`Tamamlanan Gorev: ${data.stats.completedTasks}`), 25, yPosition);
  yPosition += 8;
  pdf.text(formatTurkishText(`Toplam Kullanici: ${data.stats.totalUsers}`), 25, yPosition);
  yPosition += 20;
  ...
  data.projects.forEach((project: any, index: number) => {
    ...
    pdf.setFontSize(10);
    pdf.text(formatTurkishText(`${index + 1}. ${project.name} - Durum: ${project.status} - Gorev: ${project.taskCount}`), 25, yPosition);
    yPosition += 8;
  });
  yPosition += 15;
  ...
  data.departments.forEach((dept: any, index: number) => {
    pdf.setFontSize(10);
    pdf.text(formatTurkishText(`${index + 1}. ${dept.name} - Kullanici: ${dept.userCount}`), 25, yPosition);
    yPosition += 8;
  });
