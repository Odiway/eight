...
function generateCleanProjectPDF(data: ProjectDetailsData): jsPDF {
  ...
  // Basit header
  yPosition = addProfessionalHeader(
    doc,
    'Proje Detay Raporu',
    data.project.name
  )
  ...
  addTurkishText(
    doc,
    `Durum: ${getStatusText(data.project.status)}`,
    20,
    yPosition
  )
  yPosition += 12
  ...
  if (data.project.startDate) {
    addTurkishText(
      doc,
      `Baslangic: ${data.project.startDate.toLocaleDateString('tr-TR')}`,
      20,
      yPosition
    )
    yPosition += 12
  ...
  if (data.project.endDate) {
    addTurkishText(
      doc,
      `Bitis: ${data.project.endDate.toLocaleDateString('tr-TR')}`,
      20,
      yPosition
    )
    yPosition += 12
  ...
  const taskRows = [
    [
      ...
      `${
        data.totalTasks > 0
          ? Math.round((data.todoTasks / data.totalTasks) * 100)
          : 0
      }%`,
    ],
    [
      ...
      `${
        data.totalTasks > 0
          ? Math.round((data.inProgressTasks / data.totalTasks) * 100)
          : 0
      }%`,
    ],
    [
    ...
    [
      ...
      `${
        data.totalTasks > 0
          ? Math.round((data.blockedTasks / data.totalTasks) * 100)
          : 0
      }%`,
    ],
  ]
  ...
  if (data.tasks.length > 0) {
    ...
    const taskDetailRows = data.tasks
      .slice(0, 10)
      .map((task) => [
    ...
    if (data.tasks.length > 10) {
      ...
      addTurkishText(
        doc,
        `Not: Toplam ${data.tasks.length} gorevden ilk 10'u gosterilmektedir.`,
        20,
        yPosition
      )
      yPosition += 15
    ...
    try {
      const errorData: ProjectDetailsData = {
        project: {
          ...
          description:
            'Veritabani baglantisi veya diger bir hata nedeniyle PDF olusturulamadi.',
          status: 'ERROR',
          startDate: new Date(),
          endDate: new Date(),
          createdAt: new Date(),
